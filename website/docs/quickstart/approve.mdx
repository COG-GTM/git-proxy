...
Use the GitProxy `admin` username and password to obtain a cookie. The cookie value is saved to a file (`git-proxy-cookie`):

```bash
curl -H "Content-Type: application/json" -c git-proxy-cookie -X POST \
  -d '{"username":"admin","password":"<admin-password>"}' http://localhost:8080/auth/login
```

:::info

The default `admin` account is bootstrapped on first start. Set the
`GIT_PROXY_ADMIN_PASSWORD` environment variable to choose its password. If you
don't set it, GitProxy generates a random password and prints it **once** to the
server logs on first startup — copy it from there and change it after logging in.
...
Use the GitProxy `admin` credentials to authenticate and obtain a cookie. The cookie value is saved to a file (`git-proxy-cookie`). The admin password is taken from `GIT_PROXY_ADMIN_PASSWORD`, or auto-generated and logged once on first startup (see the [note above](/docs/quickstart/approve#2-authenticate-with-the-api)):

```bash
$ npx -- @finos/git-proxy-cli login --username admin --password <admin-password>
